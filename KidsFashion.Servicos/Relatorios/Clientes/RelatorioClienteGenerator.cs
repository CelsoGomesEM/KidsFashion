...
{
    public class RelatorioClienteGenerator : IRelatorioPDF
    {
        public async Task<MemoryStream> GerarRelatorioPDF()
        {
            var servico = new ServicoCliente();

            var clientes = servico.ObterTodos();
            ...
            foreach (var item in clientes.Result)
            {
                ...
                PdfPCell cpfCell = new PdfPCell(new Phrase(item.CPF));
